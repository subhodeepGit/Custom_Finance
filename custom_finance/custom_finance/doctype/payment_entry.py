...
class PaymentEntry(AccountsController):
	...
	def validate_duplicate_entry(self):
		...
		for d in self.get("references"):
			...
			if (d.reference_doctype, d.fees_category, d.payment_term) in reference_names:
...
#######################################################################################################################	4th change 	
		try:
			for paid in self.paid_from:
				# if self.paid_from and not self.source_exchange_rate:
				if paid['receivable_account'] and not self.source_exchange_rate:
					...
					if not self.source_exchange_rate:
						self.source_exchange_rate = get_exchange_rate(paid['receivable_account'],self.company_currency, self.posting_date)
				...
				if self.paid_from and not self.source_exchange_rate:
					...
					if not self.source_exchange_rate:
						self.source_exchange_rate = get_exchange_rate(self.paid_from,self.company_currency, self.posting_date)		
#############################################################################################################################						
...
########################################################################################################################
	def add_party_gl_entries(self, gl_entries):
		if self.party_account:
			...
			party_gl_list=[]
			if self.payment_type=="Receive":
				for t in frappe.db.get_all('Payment Entry Reference',{"parent":self.name},["account_paid_from","fees_category",'reference_name']):
					party_gl_dict = self.get_gl_dict({
						...
						"voucher_no_vali":t['reference_name']
					}, item=self)
			...
			for t in party_gl_dict:
				for d in self.get("references"):
					cost_center = self.cost_center
					if self.payment_type=="Receive":
						if d.fees_category==t["fees_category"] and t['voucher_no_vali']==d.reference_name:
							if d.reference_doctype == "Sales Invoice" and not cost_center:
								cost_center = frappe.db.get_value(d.reference_doctype, d.reference_name, "cost_center")
							# gle = party_gl_dict.copy()
							
							gle = t.copy()
							del gle['voucher_no_vali']
							gle.update({
						...
						if t["fees_category"]!="Fees Refundable / Adjustable":
							if d.fees_category==t["fees_category"] and t['voucher_no_vali']==d.reference_name:
								...
								gl_entries.append(gle)
						elif t["fees_category"]=="Fees Refundable / Adjustable":
							if d.fees_category==t["fees_category"]:
								if d.reference_doctype == "Sales Invoice" and not cost_center:
									cost_center = frappe.db.get_value(d.reference_doctype, d.reference_name, "cost_center")
								# gle = party_gl_dict.copy()
...
################################################################
@frappe.whitelist()
def get_student_detai(args,self):
...
def set_party_account(dt, dn, doc, party_type):
	...
	###########################################################################	1st change
		...
		receivable_account=frappe.get_all("Fee Component",{"parent":doc.name},["receivable_account","fees_category","outstanding_fees","grand_fee_amount"])
